...
class PZChartMatplotlib(QWidget):
    """Mapa P‑Z interactivo.  Arrastra con el mouse."""
    sigChanged = Signal(list, list)          # (poles, zeros)

    def __init__(self, parent=None):
        super().__init__(parent)
        self.fig = Figure(figsize=(4,4)); self.ax = self.fig.add_subplot(111)
        self.ax.set_xlabel("Real"); self.ax.set_ylabel("Imaginario")
        self.ax.set_xlim(-10, 10); self.ax.set_ylim(-10, 10)
        self.ax.axhline(0, color='k', lw=0.5); self.ax.axvline(0, color='k', lw=0.5)
    ...
    def _pick(self, ev):
        if ev.artist is self.line_p: self.sel = ('p', ev.ind[0])
        elif ev.artist is self.line_z: self.sel = ('z', ev.ind[0])

    def _drag(self, ev):
        if self.sel and ev.inaxes is self.ax:
            kind, idx = self.sel
            if kind == 'p': self.poles[idx] = complex(ev.xdata, ev.ydata)
            else:           self.zeros[idx] = complex(ev.xdata, ev.ydata)
            self._draw()
